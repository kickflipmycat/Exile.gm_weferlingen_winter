/*
	Config
*/
_XG_KillConfig = 1; //1 or 2
_XG_colorVictim = "#FF0400";
_XG_ColorKiller = "#64FE2E";
_XG_ColorVW = "#c4c2c2";
_XG_ColorText = "#f2f1f1";
		...
		switch (_XG_KillConfig) do
		{
			case 1:
			{
				...
				switch (_type) do
				{
					case "Weapon":
					{
						_message = format["<t color='%5'>%1</t> <t color='%8'>was killed by</t> <t color='%6'>%2</t> <t color='%8'></t> <t color='%7'></t> <t color='%8'>with</t> <img image='%4' size='2'/>",(name _victim),(name _killer),round((position _victim) distance (position _killer)),_display,_XG_colorVictim,_XG_ColorKiller,_XG_ColorVW,_XG_ColorText];
					};
					case "NoWeapon":
					{
			...
			case 2:
			{
				switch (_type) do
				{
					...
					{
						_display = getText(configFile >> "CfgWeapons" >> (currentWeapon _killer) >> "picture");
						_message = format["<t color='%4'>%1</t> <img image='%3' size='2'/> <t color='%6'>%2</t> <t color='%5'></t>",(name _victim),(name _killer),_display,_XG_colorVictim,_XG_ColorVW,_XG_ColorKiller,round((position _victim) distance (position _killer))];
					};
					case "NoWeapon":
					{
						_message = format["<t color='%3'>%1</t> <t color='%5'>was killed by</t> <t color='%4'>%2</t> <t color='%5'>without a gun!</5>",(name _victim),(name _killer),_XG_colorVictim,_XG_ColorKiller,_XG_ColorText];
